...
public class RemoveMultipleResponseHeadersGatewayFilterFactory extends AbstractGatewayFilterFactory<RemoveMultipleResponseHeadersGatewayFilterFactory.Config> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(RemoveMultipleResponseHeadersGatewayFilterFactory.class);
            ...
            response.beforeCommit(() -> {
                ...
                if(headersToRemove != null && !headersToRemove.isEmpty()) {
                    for(String header : headersToRemove) {
                        logger.debug("[RequestRateLimiter Info] header name: {}, header values: {}", header, response.getHeaders().get(header));
